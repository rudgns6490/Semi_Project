...
<script type="text/javascript">
	...
	// === 로컬 스토리지(localStorage)에 저장된 userid 값 불러와서 input 태그 userid에 넣어주기 === // 
	...
	*/		
	   ...
	   // === 로컬 스토리지(localStorage)에 userid 값 저장시키기 === // 
		if( $("input:checkbox[id=saveid]").is(":checked") ) { 
			...
			if(loginUserid != null && loginUserid != $("#loginUserid").val()) {
		...
		frm.method = "post";
		frm.action = "<%= request.getContextPath()%>/login/loginEnd.up"; 
		frm.submit();
	...
	function goEditPersonal(idx) {
		...
		// 팝업창 띄우기 
		var url = "/MyMVC/member/memberEdit.up?idx="+idx;     
	...
	function goCoinPurchaseTypeChoice(idx) {
		...
		// 코인구매 금액 선택 팝업창 띄우기 
		var url = "/MyMVC/member/coinPurchaseTypeChoice.up?idx="+idx;     
	...
	//	alert("확인용 idx : "+idx+", coinmoney : "+coinmoney);
		...
		// 아임포트 결제 팝업창 띄우기
		var url="/MyMVC/member/coinPurchaseEnd.up?idx="+idx+"&coinmoney="+coinmoney;
