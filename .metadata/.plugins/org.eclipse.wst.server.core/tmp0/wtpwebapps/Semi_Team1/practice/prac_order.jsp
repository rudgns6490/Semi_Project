...
<% 
	...
	/* int paymoney = (int)session.getAttribute("totalPrice"); */
%>
...
<script type="text/javascript">
		...
		$("#myReserve").blur(function(){
			...
			var regExp =  /^[0-9]+$/;
	   		...
	   		if(!result){
				...
				return;
	   		} else if(myreserve > parseInt("<%= session.getAttribute("totalPrice")%>")){
				alert("구매하려는 상품의 금액보다 큰 적립금은 사용할 수 없습니다.")	
				return;
			} else {
				$("#useReserve").val(myreserve.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
	...
	function goSelectDel(fk_email) {
		var url = "<%= request.getContextPath() %>/shop/post.dog";
   		window.open(url, "placeList", "left=120px, top=100px, width=500px, height=600px" );
   	};//goSelectDel----------
  	...
  	/*	
   		// 배송정보 - 주문하기 클릭시 연락처2이 비어있을 경우 자동으로 연락처1 가져오기
   		if($("#tel04") != null && $("#tel05") != null && $("#tel06") != null){
   			
   			var phoneNum4 = $("#tel04").val(); 
   	   		var phoneNum5 = $("#tel05").val(); 
   	   		var phoneNum6 = $("#tel06").val(); 
   	   			
   	   		var phoneNum1 = phoneNum4  + phoneNum5  + phoneNum6;

   	   		var regExp = /(01[0|1|6|9|7])(\d{3}|\d{4})(\d{4}$)/g;
   	   		var result = regExp.test(phoneNum1);

   	   		if(!result){
   	   			alert("받는분 연락처에 숫자만 넣어주세요.");
   	   		}
...
<div class="container">
	<div id="cnt" class="wrap_inner">
		...
		<div id="main_title"><h1 style="font-weight:bold;">주문서</h1></div>
	 		<div id="order">
			  ...
			  <table class="table tbl_order">
					...
					<c:if test="${not empty sessionScope.basketList && sessionScope.basketList != null}">			
						<c:forEach items="${basketList}" varStatus="status">
							<tbody id="tbl_body">
								<tr>
									<td style="width:100px;"><img src="${basketList[status.index].product_explanation}" width=80px; height=80px;/></td>
									<td style="text-align:left;">${basketList[status.index].product_name}</td>
									<td>${basketList[status.index].quantity}개</td>
									<td style="text-weight:bold;"><fmt:formatNumber value="${priceList[status.index].productPrice}" pattern="###,###" />원</td> 
									<td>${priceList[status.index].saveReserve}</td>
								</tr>		
							</tbody>
						</c:forEach>
						...
						<c:forEach items="${priceList}" var="price">
							<tfoot id="tbl_foot">
								<tr>
									...
									<td style="text-align:center;" >${price.totalReserve}원</td>
								</tr>
							</tfoot>
						</c:forEach>
					</c:if>
				...
				<div id="orderUserInfo">
					...
					<table id="tbl_info" class="table bottom2" >
						...
						<tr>
							...
							<td>	
								<div id="emailDiv" style="float:left;">
									<div class="inputText"><input type="text" id="email1" value="${requestScope.email1}" class="form-control MS_input_txt" size="30px" style="width:168px; margin:0px;"/></div>
									<div class="inputText">&ensp;@&ensp;</div>
									<div  class="inputText"><input type="text" id="email2" class="form-control MS_input_txt" size="30px" style="width:168px; margin:0 10px 0 0;" value="${requestScope.email2}"/></div>
									<div  class="inputText">
										<select name="mail" id="email3" onchange="inputEmail();" >  	
						...
						<tr>
							...
							<td>
								<div>
								<div class="inputText"><input type="text" name="receiverPhone1" value="${requestScope.phone1}" class="tel form-control MS_input_txt" maxlength="4" style="margin: 0 5px 0 0px;"  name="order_form_tel01"/></div>
								<div class="inputText">-</div>
								<div class="inputText"><input type="text" name="receiverPhone2" value="${requestScope.phone2}" class="tel form-control MS_input_txt" maxlength="4" name="order_form_tel02"/></div>
								<div class="inputText">-</div>
								<div class="inputText"><input type="text" name="receiverPhone3" value="${requestScope.phone3}" class="tel form-control MS_input_txt" maxlength="4" name="order_form_tel03"/></div>
								</div>
							</td>
						</tr>
					...
					<table id="tbl_delInfo" class="table bottom2" >
						...
						<tr>
							...
							<td colspan="3" style="text-align:left;">  <!-- 최근배송지=배송지목록에 들어가 선택해야함  -->
								<label for="del_place1"><input type="radio" name="del_place" class="delRadio" id="del_place1" checked="checked" style="margin: 3px 0 4px 0;"/> 기본배송지
									 <button type="button" class="btn btn-default btn_info" onclick="goSelectDel();">배송지목록</button></label>
								<label for="del_place2"><input type="radio" name="del_place" class="delRadio" id="del_place2"/> 회사</label>
								<label for="del_place3"><input type="radio" name="del_place" class="delRadio" id="del_place3"/> 자택</label>
						...
						<tr >
							...
							<td colspan="3" class="addressBox" style="vertical-align: bottom;">
								<div>
									<div class="inputText"><input type="text" name="postcode" id="postcode" class="form-control MS_input_txt" style="width:100px;" value=""/></div>
						...
						<tr>
							<td colspan="3" class="addressBox" style="border-top:0px; vertical-align: top;">
								<div>
									<div class="inputText"><input type="text" name="addr1" id="addr1" value="" class="form-control MS_input_txt" style="width:260px; margin:0 2px 0 0;"/></div>
									<div class="inputText"><input type="text" name="addr2" id="addr2" value="" class="form-control MS_input_txt" style="width:260px;"/></div>
								</div>	
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="3"><textarea class="form-control" cols="50" rows="5" id="comment" maxlength="100" style="width:450px; "></textarea></td>
						</tr>
						
						<tr>
							<th>무통장 입금자명</th>
							<td colspan="3" style="text-align:left;"><input type="text" class="form-control" style="width:200px; margin:0 6px 0 0; display: inline-block;" /><span>(주문자와 같을 경우 생략)</span></td>
						</tr>
					</table>
				...
				<table class="table top bottom2" id="tblPrice">
				    ...
				    <tbody id="tbl_body2">
				    	<tr>
				       	 <td><fmt:formatNumber value="${sessionScope.totalPriceNodel}" pattern="###,###,###"/>원</td>
				       	 <td><img src="../images/basketOrder/plus.png" class="calIcon"/><fmt:formatNumber value="${sessionScope.shippingFee}" pattern="###,###,###"/>원</td>
				       	 <td><img src="../images/basketOrder/minus.png" class="calIcon"/><span style="color:red;"><input type="text" id="useReserve" value="0" style="border:none; width:35px; text-align:right;"/>원</span></td>
				       	 <td><img src="../images/basketOrder/plus.png" class="calIcon"/><input type="text" value="0" style="border:none; width:35px; text-align:right;"/>원</td>
				       	 <td><img src="../images/basketOrder/equal.png" class="calIcon"/><span style="color:red;"><fmt:formatNumber value="${sessionScope.totalPrice}" pattern="###,###,###" /></span>원</td>
				        </tr>
				    </tbody>
				    ...
				    <tfoot>
				    	<tr>
				    		<th>적립금 사용</th>
				    		<td colspan="4"><input type="text" id="myReserve"/><span style="font-size: 9pt;">&nbsp;&nbsp;(보유 적립금 : <fmt:formatNumber value="${requestScope.reserve}" pattern="###,###,###"/>원)</span></td>
				    	</tr>
				    </tfoot>
		...
		<!--  	    <tfoot id="tbl_foot">
						<tr >
						  <th>쿠폰사용</th>
						  <th colspan="4" style="text-align: left;"><input type="text"/><button type="button" class="btn btn-sm btn-warning">쿠폰선택</button></th>
						</tr>   	
					</tfoot>	-->	
				  </table>
				</div> <!-- priceInfo -->
				...
				<div id="escrowInfo">
					<div class="tblTop">
						...
						<span id="sameEscrow"><label for="escrowCheck"><input type="checkbox" id="escrowCheck" value="" style="display:inline-block;"/>선택하신 결제수단을 다음에도 적용</label></span>			
						</h4>
					</div>
					...
					<div id="escrowInfo_detail">		
						...
						<tr>
							...
							<td >
								<ul id="method_li">
									<li><label for="escrow1"><input type="radio" name="escrow_check" id="escrow1" checked="checked">카카오페이(KAKAOPAY)</label></li>
									<li>
										<label for="escrow2"><input type="radio" name="escrow_check" id="escrow2">가상계좌</label>
									...
									<li><label for="escrow3"><input type="radio" name="escrow_check" id="escrow3">신용카드</label></li>
								</ul>
							</td>
				...
				<div id="lastPrice">
					...
					<div id="agree" class="top">				
						<table class="table" id="tblAgree">
							<tr class="bottom">
							 	<th>주문 동의</th>
							 	<td><label for="agreeCheck"><input type="checkbox" value="" id="agreeCheck"/><span>상기 결제정보를 확인하였으며, 구매진행에 동의합니다.</span></label></td>
							</tr>
							
							<tr class="bottom">
							<th>최종 결제금액</th>
							<td style="text-align: left; " ><span style="color:red; font-size: 14pt; font-weight: bold;" ><fmt:formatNumber value="${sessionScope.totalPrice}" pattern="###,###,###" /></span><span style="font-weight: bold; font-size: 14pt; ">원</span>
								<span style="color:#777; font-size:9pt;">(적립예정 : <fmt:formatNumber value="${sessionScope.saveReserve}" pattern="###,###,###"/>원)<br/>(적립 예정금액과 최종 적립금액은 상이할 수 있습니다. 주문 완료 시 지급되는 적립금을 확인해주시기 바랍니다.)</span></td>
							</tr>
					</table>	
				</div>	
				</div>	<!--lastPrice  -->
				...
				<a href=""><button type="button" onclick="goPayment();" class="btn btn_last lastBtn" style="background-color: #42362f; color:white;">주문하기</button></a>
				</div>
			</div>  <!-- order -->
			</div>
			</form>
