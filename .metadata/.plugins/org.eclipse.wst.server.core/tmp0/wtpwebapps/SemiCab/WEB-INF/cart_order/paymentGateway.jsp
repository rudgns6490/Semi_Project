...
$(document).ready(function() {
   ...
   IMP.init('imp41974598');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드".  
   ...
   IMP.request_pay({
       ...
       amount : 100,	  //'${paymentPrice}' '${coinmoney}' 현재는 100원으로 test 한다.  결제 금액 number 타입. 필수항목. 
       buyer_email : '',  // 구매자 email
       buyer_name : '${(sessionScope.loginuser).username}',	  // 구매자 이름 
       buyer_tel : '${(sessionScope.loginuser).mobile}',    // 구매자 전화번호 (필수항목)
       buyer_addr : '',  
       buyer_postcode : '',
       m_redirect_url : '192.168.50.45:9090/SemiCab/cart_order/main.ej'   
   }, function(rsp) {
       /*
		   if ( rsp.success ) {
			   var msg = '결제가 완료되었습니다.';
	   ...
	   */
		...
		*/
		//	opener.location.href = "javascript:goCoinUpdate('${idx}','${coinmoney}');";   				
			window.opener.goBilgInsert('${memberNum}','${paymentPrice}');  
		//	결재를 했으면 DB에 코인을 올려줄 함수를 호출하는 것이다.
